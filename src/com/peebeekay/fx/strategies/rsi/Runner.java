...
//		intervals.add(Interval.M1); // temporary to test if ohlc data is being retrieved without error
		...
		new RsiTrader("test1", fx, ip, fx, tDD, ohlcDD, interval, pairs.get(0), defStop, 1).run();
		new RsiTrader("test2", fx, ip, fx, tDD, ohlcDD, interval, pairs.get(1), defStop, 1).run();
		new RsiTrader("test3", fx, ip, fx, tDD, ohlcDD, interval, pairs.get(2), defStop, 1).run();
		new RsiTrader("test4", fx, ip, fx, tDD, ohlcDD, interval, pairs.get(3), defStop, 1).run();
		new RsiTrader("test5", fx, ip, fx, tDD, ohlcDD, interval, pairs.get(4), defStop, 1).run();
		new RsiTrader("test6", fx, ip, fx, tDD, ohlcDD, interval, pairs.get(5), defStop, 1).run();
